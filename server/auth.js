...
/** Returns {id, username, role} for valid Basic credentials, else null. Async:
 *  password verification runs off the event loop (see password.js). Never throws. */
export async function authenticate(req) {
  ...
  if (!row || !(await verifyPassword(password, row.pw_salt, row.pw_hash))) return null;
